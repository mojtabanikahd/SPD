# This file is an edited version retrieved from the DiffGraph package (Dtrace.R file)
...
# This funciton solve the LASSO penalized Dtrace loss function and return all the intermediate results
edited_Dtrace = function(X, lambda, covType = "pearson", tol = 1e-5, maxiter = 500){
  # number of states
  G = length(X)
  # number of genes
  p = dim(X[[1]])[2]
...
Dtrace.solve = function(S, lambda,  tol=1e-5, maxiter=500, alpha=1){
  ...
  for (i in seq(maxiter)){
    ...
    dataLogDelta <- append(dataLogDelta, list(Z))
    ...
    iteration.time <- iteration.time + iteration.stop_time-iteration.start_time
    iterationTimes <- c(iterationTimes, iteration.time)
  }
  
  out = list(delta=Delta, obj_fun=obj.fun, estimated_delta_logs = dataLogDelta, iteration_times=iterationTimes)
}
